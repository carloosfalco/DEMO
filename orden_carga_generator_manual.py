...
def generar_orden_carga_manual():
    ...
    if "ida_vuelta" not in st.session_state:
    ...
    if st.button("↔️ Ida y vuelta"):
        ...
        st.session_state.ida_vuelta = True
    ...
    with st.form("orden_form"):
        ...
        origenes, destinos = [], []
        max_pares = max(num_origenes, num_destinos)

        origen_1_val, destino_1_val = "", ""
        for i in range(max_pares):
            # ORIGEN
            if i < num_origenes:
                st.markdown(f"#### 📍 Origen {i+1}")
                if st.session_state.ida_vuelta and i == 1 and destino_1_val:
            ...
            if i < num_destinos:
                ...
                if st.session_state.ida_vuelta and i == 1 and origen_1_val:
                    default_destino = origen_1_val
                else:
                    default_destino = ""
                destino = st.text_input(f"Dirección Destino {i+1}", value=default_destino, key=f"destino_{i}")
